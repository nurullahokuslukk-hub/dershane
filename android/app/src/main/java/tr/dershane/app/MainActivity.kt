...
class MainActivity:Activity() {
 ...
 private fun text(s:String,size:Float=16f) { layout.addView(TextView(this).apply { text=s;textSize=size;setTextColor(android.graphics.Color.rgb(36,36,46));setPadding(0,12,0,16) }) }
 ...
 private fun button(label:String,action:()->Unit) { layout.addView(Button(this).apply { text=label;isAllCaps=false;minHeight=(52*resources.displayMetrics.density).toInt();setTextColor(android.graphics.Color.rgb(109,72,196));background=android.graphics.drawable.GradientDrawable().apply{setColor(android.graphics.Color.rgb(239,233,249));cornerRadius=16*resources.displayMetrics.density};setOnClickListener { action() } }) }
 private fun runNetwork(action:()->Unit){if(isFinishing||isDestroyed)return;executor.execute{try{action();runOnUiThread{if(!isFinishing&&!isDestroyed)show()}}catch(e:Exception){runOnUiThread{if(!isFinishing&&!isDestroyed){if(e is ApiError&&e.status==401&&state.token.isNotEmpty())SessionRecovery.expire(this,state) else AlertDialog.Builder(this).setTitle("İşlem tamamlanamadı").setMessage(if(e is ApiError&&e.code=="PENDING_REVOCATION")"Bekleyen paylaşım iptalini tamamlamak için önceki öğrenci hesabıyla giriş yapın." else if(e is ApiError)"Sunucu: ${e.code}" else "Adres, bağlantı ve giriş bilgilerini kontrol edin.").setPositiveButton("Tamam",null).show()}}}}}
 private fun show() {
  ...
  if(state.token.isEmpty()) {
   ...
   button("Giriş yap") { val url=base.text.toString().trim();val institution=tenant.text.toString().trim();val login=user.text.toString().trim();val password=pass.text.toString();runNetwork { val previousStudent=state.prefs.getString("student","");val pending=state.prefs.getBoolean("pendingRevoke",false);state.prefs.edit().putString("base",url).commit();val result=Api(state).request("/auth/login",JSONObject().put("tenant",institution).put("login",login).put("password",password));state.token=result.getString("accessToken");val me=Api(state).request("/me");if(me.getString("role")!="student") {state.token="";throw IllegalArgumentException("Student only")};if(pending){if(previousStudent!=me.getJSONObject("student").getString("id")){try{Api(state).request("/auth/logout",JSONObject())}finally{state.token=""};throw ApiError(409,"PENDING_REVOCATION")};Api(state).request("/consent",JSONObject().put("enabled",false).put("noticeVersion","2026-09-v1"))};Queue(this).use { it.purge() };state.enabled=false;state.prefs.edit().putString("student",me.getJSONObject("student").getString("id")).putString("name",me.getString("name")).putString("installation",UUID.randomUUID().toString()).putBoolean("pendingRevoke",false).commit() } }
   text("Telefon izni olmadan da çalışma kaydı tutulabilir. Kullanım paylaşımı girişten sonra ayrı seçilir.",14f);return
  }
  button("Öğrenci alanına dön") { startActivity(Intent(this,StudentActivity::class.java));finish() }
